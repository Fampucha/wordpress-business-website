 ...
 * @return void
 */
function eko_plastics_register_acf_gravity_form_field() {
	if ( ! class_exists( 'acf_field' ) ) {
		return;
	}

	if ( class_exists( 'Eko_Plastics_ACF_Field_Gravity_Form' ) ) {
		return;
	}
	 ...
	 */
	class Eko_Plastics_ACF_Field_Gravity_Form extends acf_field {
		...
		public function initialize() {
			...
			$this->label    = __( 'Gravity Form', 'eko-plastics' );
			$this->category = 'choice';
			$this->defaults = array(
				'allow_null'  => 1,
				'placeholder' => __( 'Select form', 'eko-plastics' ),
				'ui'          => 1,
			);
		}
		...
		public function render_field( $field ) {
			...
			if ( ! class_exists( 'GFAPI' ) ) {
				...
				echo '<p>' . esc_html__( 'Gravity Forms plugin is not active.', 'eko-plastics' ) . '</p>';
			...
			if ( empty( $choices ) ) {
				...
				echo '<p>' . esc_html__( 'No Gravity Forms found.', 'eko-plastics' ) . '</p>';
	...
	acf_register_field_type( 'Eko_Plastics_ACF_Field_Gravity_Form' );
}
add_action( 'acf/include_field_types', 'eko_plastics_register_acf_gravity_form_field' );
